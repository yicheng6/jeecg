package com.jeecg.gowin.service.impl;
...
@Service("tGowinStudentService")
@Transactional
public class TGowinStudentServiceImpl extends CommonServiceImpl implements TGowinStudentServiceI {
	 ...
	 * @return
	 */
	private void doAddBus(TGowinStudentEntity t) throws Exception{
		//-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
	 ...
	 * @return
	 */
	private void doUpdateBus(TGowinStudentEntity t) throws Exception{
		//-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
	 ...
	 * @return
	 */
	private void doDelBus(TGowinStudentEntity t) throws Exception{
	    //-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
 	...
 	private Map<String,Object> populationMap(TGowinStudentEntity t){
		...
		map.put("xh", t.getXh());
		map.put("xm", t.getXm());
		map.put("xfzjh", t.getXfzjh());
		map.put("xbm", t.getXbm());
		map.put("csrq", t.getCsrq());
		map.put("jg", t.getJg());
	 ...
	 * @return
	 */
 	public String replaceVal(String sql,TGowinStudentEntity t){
 		...
 		sql  = sql.replace("#{xh}",String.valueOf(t.getXh()));
 		sql  = sql.replace("#{xm}",String.valueOf(t.getXm()));
 		sql  = sql.replace("#{xfzjh}",String.valueOf(t.getXfzjh()));
 		sql  = sql.replace("#{xbm}",String.valueOf(t.getXbm()));
 		...
 		if(StringUtil.isNotEmpty(cgJavaValue)){
			...
			try {
				...
				if(obj instanceof CgformEnhanceJavaInter){
					...
					javaInter.execute("t_gowin_student",data);
				}
			} catch (Exception e) {
				e.printStackTrace();
